...
* *e0c, e0d.* LIF di rete cluster di hosting
* *e0e.* Hosting della LIF RSM
* *e0f.* Hosting dell'interconnessione ha LIF
...
La rete interna di un cluster multinode può essere validata utilizzando la funzionalità di verifica della connettività di rete. Questa funzione può essere richiamata dall'interfaccia CLI di deploy che esegue il `network connectivity-check start` comando.
