...
È possibile accedere alla pagina della documentazione online utilizzando l'IP o il nome di dominio della macchina virtuale di implementazione. Per visualizzare la pagina, immettere un URL con il seguente formato nel browser (sostituendo l'indirizzo IP o il nome di dominio appropriati per l'istanza di distribuzione della macchina virtuale): `\http://<ip_address>/api/ui`
